...
class OthelloABSearch(object):
    ...
    def __init__(self, position_str):
        ...
        self.othello_state = OthelloState(self._max_player, self._root_position, self._othello_move, 0,2, True)
        np.random.seed(42)

    # AB SEARCH
    def absearch( self, othello_state ):
        print("absearch\n", othello_state.curr_position.get_moves())
        max_move = self._max_value(othello_state, -np.inf, np.inf)
        print("absearch complete", max_move, max_move.path)

    def _max_value(self, othello_state, alpha, beta):
        if(self._terminal_test(othello_state)):
            return self._utility(othello_state)
        moves = othello_state.get_moves()
        max_move = OthelloMove(value=-np.inf)
        for move in moves:
            result = self._result_state(othello_state, move)
            move = self._min_value( result, alpha, beta)#.sort(key=lambda x: x.value, reverse=True)
            move.value = move.path[0].value
    ...
    def _min_value(self, othello_state, alpha, beta):
        ...
        for move in moves:
            ...
            result = self._result_state(othello_state, move)
            move = self._max_value( result, alpha, beta)#.sort(key=lambda x: x.value)
            move.value = move.path[0].value
